...
and interpretable bridge: a mapping from high-frequency trading signatures to predictable post-event return dynamics in the earnings setting. We establish that information-induced and liquidity-induced pressures are empirically separable and economically meaningful; the predictive regressions serve as validation that the decomposition captures real information, not as a proposed trading strategy.

\paragraph{Contributions.}
This paper contributes to the literature in three ways. First, we introduce a microstructure-based framework that decomposes pre-announcement trading pressure into economically interpretable components using high-frequency data. Second, we provide empirical evidence that the microstructure feature space exhibits a latent structure consistent with information-driven and liquidity-driven trading by applying principal component analysis. Third, we show that these PCA-derived factors are not merely descriptive, but have direct predictive power for post-earnings returns.

At a conceptual level, the paper distinguishes between the magnitude of information (captured by earnings surprises) and the timing of information incorporation (captured by trading pressure), providing a unified framework for understanding post-earnings return dynamics.
