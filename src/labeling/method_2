# Method 2: Separate PI and RI. PI and RI are the weighted mean of the components. Weights are determined by reliability(noise-lessness).
import numpy as np
import pandas as pd
...
def _event_pi_ri(ofi_series: pd.Series) -> dict:
    ...
    pi_wts  = np.array([w_rho, w_c, w_d], dtype=float)

    valid_pi = ~np.isnan(pi_vals)
    PI = (
        np.sum(pi_vals[valid_pi] * pi_wts[valid_pi]) /
        np.sum(pi_wts[valid_pi])
        if valid_pi.any()
        else np.nan
    )
    ...
    ri_wts  = np.array([w_gamma, w_f, w_a], dtype=float)

    valid_ri = ~np.isnan(ri_vals)
    RI = (
        np.sum(ri_vals[valid_ri] * ri_wts[valid_ri]) /
        np.sum(ri_wts[valid_ri])
        if valid_ri.any()
        else np.nan
    )
